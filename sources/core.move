...
module overmind::breeder_core {
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        let breed_dragons_events = account::new_event_handle<BreedDragonsEvent>(&account_resource_signer);
        let hatch_dragon_events = account::new_event_handle<HatchDragonEvent>(&account_resource_signer);
        let collections = simple_map::create<String, DragonRace>();
        let ongoing_breedings = simple_map::create<vector<u8>, u64>();

        let breeder = Breeder {
            collections,
            ongoing_breedings,
            create_dragon_collection_events,
            create_dragon_events,
            breed_dragons_events,
            hatch_dragon_events
        };

        let collections = simple_map::create<String, SwordType>();
        let create_sword_collection_events = account::new_event_handle<CreateSwordCollectionEvent>(&account_resource_signer);
        let create_sword_events = account::new_event_handle<CreateSwordEvent>(&account_resource_signer);
        let combine_swords_events = account::new_event_handle<CombineSwordsEvent>(&account_resource_signer);


        let combiner = Combiner {
            collections,
            create_sword_collection_events,
            create_sword_events,
            combine_swords_events
        };
    ...
    */
    public entry fun create_dragon_collection(
        ...
        simple_map::add(&mut state.breeder.collections, name, dragon_race);
    ...
    */
    public entry fun create_sword_collection(
        ...
        let properties_sum = calculate_swords_starting_properties_sum(combine_amount);

        // TODO: Assert that sum of provided property_values is correct
        assert_sword_property_values_sum_is_correct(&property_values, properties_sum);
        ...
        // TODO: Create a collection
        create_collection_internal(_account, name, description, uri, SWORD_COLLECTION_MAX_SUPPLY, false, false);
    ...
    */
    ...
    public fun get_equipment_starting_properties_sum(combine_amount: u64): u64 {
        ...
        let value = calculate_swords_starting_properties_sum(combine_amount);
        value
    }
    ...
    */
    ...
    inline fun assert_signer_is_admin(admin: &signer) {
        ...
        let admin_address = signer::address_of(admin);
        assert!(@admin == admin_address, ERROR_SIGNER_NOT_ADMIN);
    }
    ...
    inline fun assert_signer_owns_token(owner: &signer, token: Object<Token>) {
        ...
        assert!(owner_address == object::object_address(&token), ERROR_SIGNER_IS_NOT_THE_OWNER);
    }
